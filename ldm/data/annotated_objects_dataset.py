...
from ldm.data.conditional_builder.objects_bbox import ObjectsBoundingBoxConditionalBuilder
from ldm.data.conditional_builder.objects_center_points import ObjectsCenterPointsConditionalBuilder
from ldm.data.conditional_builder.utils import load_object_from_string

from ldm.data.helper_types import BoundingBox, CropMethodType, Image, Annotation, SplitType
from ldm.data.image_transforms import (CenterCropReturnCoordinates,
...
class AnnotatedObjectsDataset(Dataset):
    def __init__(self,
        ...
        self._conditional_builders = None  # bbox条件生成器
    ...
    @property
    def conditional_builders(
            self) -> ObjectsCenterPointsConditionalBuilder:  # cannot set this up in init because no_classes is only known after loading data in init of superclass
        if self._conditional_builders is None:
            self._conditional_builders = {
                'objects_center_points': ObjectsCenterPointsConditionalBuilder(
                    ...
                    getattr(self, 'use_additional_parameters', False)  # getattr() 函数用于返回一个对象属性值,设置了默认值False。
                ),
    ...
    def clean_up_annotations_and_image_descriptions(self) -> None:
        ...
        print("clean_up_annotations_and_image_descriptions")

    @staticmethod
    def filter_object_number(all_annotations: Dict[str, List[Annotation]],
                             min_object_area: float,
                             min_objects_per_image: int,
                             max_objects_per_image: int) -> Dict[str, List[Annotation]]:
        filtered = {}
        ...
        print("一共过滤了", len(all_annotations) - len(filtered), "张图片")
        return filtered
    ...
    def __getitem__(self, n: int) -> Dict[str, Any]:
        ...
        try:
            ...
            for conditional, builder in self.conditional_builders.items():
                #  objects_center_points or  objects_bbox
                if conditional in self.keys:
                    sample[conditional] = builder.build_dataset(sample['annotations'],
                                                                sample['crop_bbox'],
                                                                sample['flipped'])
                else: #自己加的，防止bug
                    continue
        except Exception as e:
            print(f'Error while loading sample {n} with image_id {image_id} and image_path {sample["image_path"]}')
